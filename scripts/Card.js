...
class Card {
  ...
  _getTemplate() {
    ...
    this._likeButtonElement = this._cardElement.querySelector('.element__like-button');
...
//слушатели событий в сгенерированной карточке 
  _setEventListeners() {
    this._cardImageElement.addEventListener('click', () => this._handleOpenFullImage());
    this._likeButtonElement.addEventListener('click', () => this._handleLikeButton());
    this._cardDeleteButton.addEventListener('click', () => this._deleteCardElement());
  }
};
